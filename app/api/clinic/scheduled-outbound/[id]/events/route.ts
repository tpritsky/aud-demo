...
/**
 * GET /api/clinic/scheduled-outbound/[id]/events — audit timeline for one scheduled row.
 */
export async function GET(
  ...
  try {
    ...
    if (role === 'member' && row.created_by !== user.id) {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
    }
    ...
    console.error('GET scheduled-outbound events', e)
