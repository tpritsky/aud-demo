...
function targetUserIdFromRequest(request: NextRequest): string | null {
  return new URL(request.url).searchParams.get('userId')?.trim() || null
}

/**
 * GET/POST/PATCH /api/super-admin/user-scheduled-outbound?userId=<target>
 * Same semantics as /api/clinic/scheduled-outbound for the target user's clinic (super_admin only).
 * POST sets created_by to the target user id.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('GET super-admin user-scheduled-outbound', e)
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('POST super-admin user-scheduled-outbound', e)
    return NextResponse.json({ error: 'Something went wrong' }, { status: 500 })
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (role === 'member') {
      q = q.eq('created_by', targetUserId)
    }
    ...
    console.error('PATCH super-admin user-scheduled-outbound', e)
    return NextResponse.json({ error: 'Something went wrong' }, { status: 500 })
  }
}
